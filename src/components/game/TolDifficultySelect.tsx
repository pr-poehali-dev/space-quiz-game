...
interface TolDifficultySelectProps {
  itemId: number;
...
const TolDifficultySelect = ({ itemId, solvedDifficulties, onSelect, onBack }: TolDifficultySelectProps) => {
  const [visible, setVisible] = useState(false);
  const itemIndex = truthOrLieData.findIndex(t => t.id === itemId);
      ...
      <div className="w-full max-w-md">
          ...
          <div className="mt-3 flex justify-center gap-2">
            {difficulties.map(d => (
              <div
                ...
                style={{
                  background: solvedDifficulties.includes(d) ? TOL_DIFFICULTY_CONFIG[d].color : 'rgba(255,255,255,0.15)',
                  boxShadow: solvedDifficulties.includes(d) ? `0 0 8px ${TOL_DIFFICULTY_CONFIG[d].color}` : 'none',
                }}
              />
            ))}
          </div>
        ...
        <div className="flex flex-col gap-4">
          {difficulties.map((diff, idx) => {
            const config = TOL_DIFFICULTY_CONFIG[diff];
...
export default TolDifficultySelect;
